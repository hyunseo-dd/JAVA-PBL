...
public class TodoApp {
    public static void main(String[] args) {
        ...
        System.out.println("\n---7. 할 일 수정 (선택 가능 목록 표시) ---");
        for (TodoItem item : manager.getAllItems(false)) {
            System.out.println(item);
        }
        System.out.println("\n---7-1. 실제 수정 실행 ---");
        TodoItem updateTarget = manager.findItemById(task2.getId());
        if (updateTarget != null) {
            updateTarget.updateItem("주간 회의 자료 정리", 3, tomorrow);
        }
        }
        System.out.println("\n---8. 수정 후 목록 확인 ---");
        for (TodoItem item : manager.getAllItems(false)) {
        ...
        System.out.println("\n---9-1. 실제 검색 실행 ---");
        List<TodoItem> searchResults = manager.searchItems("회의");
        for (TodoItem item : searchResults) {
            System.out.println(item);
        }
        System.out.println("\n---10. 할일 삭제 ---");
        for (TodoItem item : manager.getAllItems(false)) {
        ...
        System.out.println("\n---10-1. 실제 삭제 실행 ---");
        manager.deleteItem(task3.getId());
        System.out.println("\n---11. 삭제 후 최종 할 일 목록 ---");
        for (TodoItem item : manager.getAllItems(false)) {
            System.out.println(item);
        }
